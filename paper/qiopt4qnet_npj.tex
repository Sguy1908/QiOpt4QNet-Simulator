...
zero distance falls to $0.048$ bit/pulse (a $51\,\%$ reduction) and is
non-monotonic in distance, rising to $0.056$ at 5~km before falling, as in their
Fig.~8. The noise-aware rate never exceeds the ideal rate at any distance; the penalty
relative to it falls below $5\,\%$ by 20~km and to about $1\,\%$ at 30~km.
...
\subsection*{A noise-blind decoy estimator over-certifies only marginally}
...
compared it with a noise-aware bound in which the parasitic source is
characterised, its clicks are removed from the decoy statistics, and every
noise-induced error is charged to the single-photon key bits (Methods). By
construction the aware rate never exceeds the ideal rate. With
Table~\ref{tab:params} the naive rate is \emph{higher} than the aware one at
every distance, in the direction the paper predicts, but by at most
$0.74\,\%$ (where the rate exceeds $10^{-4}$ bit/pulse; $0.68\,\%$ at 0~km,
$0.64\,\%$ at 5~km and $<0.01\,\%$ at 50~km). The key-rate loss in
Fig.~\ref{fig:keyrate} therefore comes almost entirely from the added QBER, not
from mis-estimation. The paper reports no explicit expression for this case, so
this is our reconstruction; we do not carry the estimator bias into the network
study.
...
topologies and 10 seeds; the ``clean'' scenario emits no light, so it has no
insecure links.}
\label{fig:network}
\end{figure}
...
$\Delta F\approx5\times10^{-5}$ (between $3\times10^{-6}$ and $2\times10^{-4}$
across instances), yet the greedy heuristics
...
+Y_0e_0^2e_ds_cn_c,
\end{align}
with $e_0=1/2$ (the paper's Eqs.~23--24). The exact single-photon yield $Y_1$ and
error mass $E_1Y_1$ in the presence of noise follow by setting the signal photon
number to one in the same construction (Eq.~66 of the paper, averaged over the
noise Poisson law).

\subsection*{Noise-aware key rate}
The parasitic source is characterised, so its clicks are removed from the decoy
statistics and the single-photon contribution $Q_1=s\,e^{-s}Y_1^L$ is the same
bound as in the noise-free analysis; noise adds errors but no key. Charging all
of them to the single-photon key bits,
\begin{equation}
e_1=\min\Bigl\{\tfrac12,\ e_1^U+\frac{(E_1Y_1)_{\muEL}-(E_1Y_1)_{0}}{Y_1^L}\Bigr\},
\qquad
R=q\Bigl\{Q_1\bigl[1-\hbin(e_1)\bigr]-Q_sf_{\mathrm{EC}}\hbin(E_s)\Bigr\},
\end{equation}
with $Q_s,E_s$ the observed noisy signal gain and QBER. This rate equals the
ideal one at $\muEL=0$ and is never larger than it at any distance (a regression
test checks $0$--$340$~km at three drive levels and $\muEL=0.3,1.0$, and the
50~km case explicitly).
